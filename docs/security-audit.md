...
| G5 ✔ | M | ~~Aucun endpoint de changement ni de réinitialisation de mot de passe.~~ **Corrigé** : `PATCH /users/{id}/password`, propriétaire (avec `currentPassword`) ou `ADMIN` (sans) — ADR §11 addendum "Mot de passe". **Reste assumé** : pas de flux "mot de passe oublié" par email (pas d'infra mail), rotation du mot de passe bootstrap toujours Vault + suppression (soft) de la ligne admin (ADR §1 addendum). | identity | fait, reset par email pas prévu |
| G6 ✔ | B | ~~`show-details: always` dans payment et travel.~~ **Corrigé** : `never` dans les trois services (§4). | payment, travel | fait |
| G7 ✔ | M | ~~Aucun en-tête de sécurité HTTP.~~ **Corrigé** : middleware Traefik (`nosniff`, `frameDeny`, `referrerPolicy`, HSTS) sur les 4 routers (identity/payment/travel/admin — ADR §11 addendum). **Pas de CSP** : écrite quand les origines de scripts du front seront stabilisées. | bord (Traefik) | fait, CSP à faire |
| G8 | B | Jeton en `localStorage` (vol par XSS) ; pas de cookie `HttpOnly`. Le refresh token (G10) est stocké de la même façon — même exposition, fenêtre plus longue (7 j au lieu de 15 min). | front | assumé (Phase 8), aggravé par G10 |
...
| G10 ~ | B | **Mitigation partielle** (ADR §11 addendum "Refresh token") : un refresh token opaque, rotatif (7 j, révoqué à chaque usage) permet de renouveler la session sans réintroduire ses identifiants, et `POST /auth/logout` le révoque. **Toujours assumé** : le JWT d'accès (15 min) lui-même reste non révocable — un token déjà émis reste valide jusqu'à expiration même après logout, changement de mot de passe, ou suppression de compte (payment/travel n'ont toujours pas accès à `users`) ; clé HS256 partagée entre les trois services ; pas de "reuse detection" en chaîne sur le refresh token. | tous | mitigé, pas fermé |
...
| G12 ✔ | B | ~~Emails non normalisés (casse) : `A@x.com` et `a@x.com` étaient deux comptes.~~ **Corrigé** : `trim` + minuscule appliqué à la création, au login et à la mise à jour d'email (`EmailNormalizer`, même règle que `LoginThrottle` — ADR §11 addendum "Normalisation des emails"). **Reste assumé** : les comptes créés avant ce correctif avec des casses différentes ne sont pas fusionnés rétroactivement ; `POST /users` répond toujours 409 sur un email déjà pris (**énumération de comptes** possible à l'inscription — le login, lui, n'énumère pas). | identity | fait, pas de migration rétroactive |
